...
URL = 'https://consumer.fda.gov.tw/LSPN/CsvDownload?nodeId=94'
SAMPLE = 'tfda_cosmetics_gmp.csv'
...
def crawl() -> pd.DataFrame:
    ...
    try:
        ...
        logging.warning('TFDA cosmetics GMP fetch failed: %s; using sample', e)
        try:
            csv_data = utils.load_sample(SAMPLE)
        except Exception:
            return pd.DataFrame(columns=['公司名稱', '電話', 'Email', '官網', '分類'])

    tmp_path = Path('/tmp/tfda_cosmetics_gmp.csv')
