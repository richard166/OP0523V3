...
from .sources import four_a, google_places, tfda_health_gmp, tfda_cosmetic_gmp, website_email
...
def main() -> None:
    ...
    try:
        datasets.append(pd.DataFrame(tfda_health_gmp.fetch()))
    except Exception as e:
        logging.warning('tfda_health_gmp failed: %s', e)
    try:
        datasets.append(pd.DataFrame(tfda_cosmetic_gmp.fetch()))
    except Exception as e:
        logging.warning('tfda_cosmetic_gmp failed: %s', e)
